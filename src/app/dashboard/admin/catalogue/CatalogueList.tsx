...
import { BookOpen, School, Building2, GraduationCap, Trash2 } from 'lucide-react';
...
export default function CatalogueList({ colleges }: { colleges: any[] }) {
  ...
  const handleDelete = async (
    ...
    if (
      !confirm(
        `Delete ${noun} “${label}”?${
          type !== 'course' ? ' This also removes nested catalogue items under it.' : ''
        }`
      )
    ) {
      return;
    ...
    const endpoint =
      type === 'college'
        ? '/api/admin/catalogue/colleges'
        : type === 'dept'
          ? '/api/admin/catalogue/departments'
          : type === 'prog'
            ? '/api/admin/catalogue/programmes'
            : '/api/admin/catalogue/classes';
    ...
    try {
      ...
      if (!res.ok) {
        alert(data.error || `Failed to delete ${noun}`);
        return;
      }
          ...
          <div key={college.id} className={styles.collegeCard}>
            <div className={styles.collegeHeader}>
              ...
              <div className={styles.headerActions}>
                <button
                  onClick={() => handleOpenModal('dept', college.id)}
                  className="btn btn-outline"
                  style={{ padding: '6px 12px', borderRadius: '8px', fontSize: '14px' }}
                >
                  Add Dept
                </button>
                <button
                  ...
                  onClick={() => handleDelete('college', college.id, college.name)}
                ...
                college.departments.map((dept: any) => (
                  <div key={dept.id} className={styles.departmentCard}>
                    <div className={styles.deptHeader}>
                      ...
                      <div className={styles.headerActions}>
                        <button
                          onClick={() => handleOpenModal('prog', dept.id)}
                          className="btn btn-outline"
                        ...
                        dept.programmes.map((prog: any) => (
                          <div key={prog.id} className={styles.programmeCard}>
                            <div className={styles.progHeader}>
                              ...
                              <div className={styles.headerActions}>
                                <button
                                  onClick={() => handleOpenModal('course', prog.id)}
                                ...
                                <button
                                  ...
                                  onClick={() => handleDelete('prog', prog.id, prog.name)}
        ...
        >
          ...
          >
            <h2 style={{ marginTop: 0, marginBottom: '1.5rem', color: '#0f172a' }}>
              {modalState.type === 'dept'
                ? 'Add Department'
                : modalState.type === 'prog'
                  ? 'Add Programme'
                  : 'Add Course'}
            </h2>
